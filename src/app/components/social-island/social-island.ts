...
@Component({
  ...
  template: `
    <aside
      class="z-40 bg-[var(--color-card)]/90 backdrop-blur-md border border-[var(--color-border)] shadow-lg flex w-fit items-center justify-center rounded-2xl transition-[bottom] duration-200 ease-out fixed left-1/2 -translate-x-1/2 flex-row p-1.5 gap-1 md:left-5 md:top-1/2 md:-translate-y-1/2 md:translate-x-0 md:flex-col md:p-2 md:gap-1.5 md:w-auto"
      style="bottom: 32px"
    ...
    >
      <button
        ...
        class="p-2.5 rounded-xl flex items-center justify-center transition-colors hover:bg-[var(--color-card-hover)] group relative shrink-0"
      ...
      <div class="bg-[var(--color-border)] shrink-0 w-px h-5 md:w-5 md:h-px" aria-hidden="true"></div>

      @for (link of socialLinks; track link.name) {
        <a
          [href]="link.url"
          [target]="link.url.startsWith('mailto') ? '_self' : '_blank'"
          [attr.rel]="link.url.startsWith('mailto') ? null : 'noopener noreferrer'"
          class="p-2.5 rounded-xl flex items-center justify-center transition-colors hover:bg-[var(--color-card-hover)] group relative shrink-0"
          [attr.aria-label]="link.name"
        >
          <svg
            class="w-5 h-5 text-[var(--color-text)] group-hover:text-[var(--color-primary)] transition-colors"
    ...
    @if (isOpen) {
      <div
        class="fixed z-[60] flex flex-col gap-1 p-3 bg-[var(--color-card)] border border-[var(--color-border)] rounded-2xl shadow-xl min-w-[240px]"
        [style.left.px]="dropdownPosition.x"
        [style.top.px]="dropdownPosition.y"
        role="menu"
        aria-label="Download resume"
      >
        <div class="flex items-center gap-2 pb-2 mb-1 border-b border-[var(--color-border)]">
        ...
        <a
          ...
          (click)="closeDropdown()"
          role="menuitem"
          class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[var(--color-card-hover)] text-sm text-[var(--color-text)] transition-colors"
        >
          <div class="flex flex-col leading-tight">
            <span class="font-medium">English (Global)</span>
            <span class="text-[10px] text-[var(--color-text-muted)]">PDF</span>
          </div>
        </a>
        <a
