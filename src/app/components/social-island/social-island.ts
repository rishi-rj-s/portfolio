  ...
  template: `
    ...
    <aside 
      ...
      @for (link of socialLinks; track link.name) {
        <a [href]="link.url" [target]="link.url.startsWith('mailto') ? '_self' : '_blank'" class="p-2 rounded-full flex items-center justify-center transition-transform hover:scale-110 active:scale-95 group relative shrink-0" [attr.aria-label]="link.name">
           <div class="absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity rounded-full {{link.bgClass}}"></div>
           <svg class="w-5 h-5 md:w-6 md:h-6 {{link.iconClass}}" [attr.fill]="link.isStroke ? 'none' : 'currentColor'" [attr.stroke]="link.isStroke ? 'currentColor' : 'none'" viewBox="0 0 24 24">
              <path [attr.stroke-linecap]="link.isStroke ? 'round' : null" [attr.stroke-linejoin]="link.isStroke ? 'round' : null" [attr.stroke-width]="link.isStroke ? '2' : null" [attr.d]="link.svgPath" />
           </svg>
        </a>
      }
    </aside>
