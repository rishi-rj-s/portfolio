  ...
  template: `
    <div class="h-full flex flex-col justify-center">
      ...
      <p class="text-sm md:text-base text-[var(--color-text-muted)] mb-8 max-w-xl">
        {{ profile.currentTitle }}. Full stack background across NestJS, Angular, React, and Next.js — {{ profile.availability }}
      </p>
      ...
      <div class="relative pl-8 border-l border-[var(--color-border)] space-y-12">
        <div class="relative group">
          ...
          <ul class="text-[var(--color-text-muted)] space-y-2 list-disc list-outside ml-4">
            <li>Shipping enterprise Angular features with a focus on scalable UI architecture and performance.</li>
            <li>Implementing robust patterns for complex business logic in a production ERP-style codebase.</li>
          </ul>
        </div>
        ...
        <div class="relative group">
          ...
          <ul class="text-[var(--color-text-muted)] space-y-2 list-disc list-outside ml-4">
            <li>Delivered client and SaaS products end-to-end (Green Power India, Tagtics) — UI, schema, APIs, deploy.</li>
            <li>Operated production backends on AWS EC2 with payments, auth, and Postgres/Mongo/Redis data layers.</li>
          </ul>
        </div>
      </div>
    </div>
